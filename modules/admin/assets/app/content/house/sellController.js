 ...
 * 出售房源管理控制器
 */
angular.module('blogAdmin').controller('SellController', ["$rootScope", "$scope", "$location", "$http", "$filter", "dataService", function ($rootScope, $scope, $location, $http, $filter, dataService) {
	$scope.id = '';
	...
	$scope.load = function () {
        ...
        dataService.getItems('admin/sell/list', p)
            .success(function (data) {
            	if (data.result == 0) {
            		$scope.setPagedItems(data);
            	}
            	else if (data.data) {
            		toastr.error('获取出售房源列表出错,原因：' + data.data[0].content[0]);
            	}
            	else {
            		toastr.error('获取出售房源列表出错');
            	}
            })
            .error(function () {
                toastr.error('获取出售房源列表出错');
	...
	$scope.get = function () {
        ...
        dataService.getItems('admin/sell/get', p)
            .success(function (data) {
            	if (data.result == 0) {
                	...
                	$scope.sales = data.data[0].content[5];//售价
                	$scope.servicefee = data.data[0].content[6];//物业费
                	$scope.linkphone = data.data[0].content[7];//联系电话               	
            	}
            	else if (data.data) {
            		toastr.error('获取出售详情出错,原因：' + data.data[0].content[0]);
            	}
            	else {
            		toastr.error('获取出售详情出错');
            	}
            })
            .error(function () {
                toastr.error('获取出售详情出错');
	...
	$scope.save = function () {
		...
		if (isNaN($scope.sales)) {
			toastr.error('售价有非法字符');
			return;
		}
		...
		var p = 
		{
			...
			body: $scope.id+'\t'+$scope.roomnum+'\t'+$scope.title+'\t'+$scope.floorcount+'\t'+$scope.area+'\t'+
					$scope.sales+'\t'+$scope.servicefee+'\t'+$scope.linkphone
		};
        dataService.getItems('admin/sell/save', p)
            .success(function (data) {
            	if (data.result == 0) {
            		toastr.success('出售信息保存成功');
            	}
            	else if (data.data) {
            		toastr.error('出售信息保存失败，原因：' + data.data[0].content[0]);
            	}
            	else {
            		toastr.error('出售信息保存失败');
            	}
            })
            .error(function () {
	...
	$scope.del = function (id) {		
        ...
        dataService.getItems('admin/sell/del', p)
            .success(function (data) {
            	if (data.result == 0) { 
            		toastr.success('出售信息删除成功');
            		//从缓存中删除记录
            		...
            		toastr.error('出售信息删除失败'); //原因：' + data[0].content[0]);
            	}
            })
            .error(function () {
                toastr.error('出售信息删除失败');
            });
	}
